 ...
 * */
function setTextFieldValuesD(name, contact_No, address, email, nic_No, license_No, license_Img, driverAvailability, user_Name, password) {
    $("#Name").val(name);
    ...
    $.ajax({
        url: driverBaseUrl + "driver/loadAllDrivers", method: "GET", dataType: "json", success: function (res) {
            ...
            for (let i of res.data) {
                ...
                let row = "<tr><td>" + user_Id + "</td><td>" + name + "</td><td>" + contact_No + "</td><td>" + address + "</td><td>" + email + "</td><td>" + nic_No + "</td><td>" + license_No + "</td><td>" + driverAvailability + "</td><td>" + role_Type + "</td><td>" + user_Name + "</td><td>" + password + "</td></tr>";
    ...
    if (event.which === 13) {
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                let row = "<tr><td>" + res.user_Id + "</td><td>" + res.name.name + "</td><td>" + res.contact_No + "</td><td>" + res.address + "</td><td>" + res.email + "</td><td>" + res.nic_No + "</td><td>" + res.license_No + "</td><td>" + res.driverAvailability + "</td><td>" + res.user.role_Type + "</td><td>" + res.user.user_Name + "</td><td>" + res.user.password + "</td></tr>";
                $("#driverTable").append(row);
            },
            error: function (error) {
                loadAllDrivers();
...
function blindClickEventsD() {
    $("#driverTable>tr").on("click", function () {
        ...
        console.log(user_Id,  name, contact_No, address, email, nic_No, license_No, driverAvailability, role_Type, user_Name,password);
...
driverValidations.push({
    reg: regExName, field: $('#Name'), error: 'Driver  Name Pattern is Wrong'
});
driverValidations.push({
...
$("#Name,#contact_No,#address,#email,#nic_No,#license_No,#user_Name,#password").on('keydown', function (event) {
    if (event.key === "Tab") {
        event.preventDefault();
    }
});

$("#Name,#contact_No,#address,#email,#nic_No,#license_No,#user_Name,#password").on('keyup', function (event) {
    checkValidity(driverValidations);
});

$("#Name,#contact_No,#address,#email,#nic_No,#license_No,#user_Name,#password").on('blur', function (event) {
    checkValidity(driverValidations);
});

$("#Name").on('keydown', function (event) {
    if (event.key === "Enter" && check(regExName, $("#Name"))) {
        $("#contact_No").focus();
    } else {
        focusText($("#Name"));
    }
