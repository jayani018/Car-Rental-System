let RentbaseUrl = "http://localhost:8080/car_Rent_Back_End_war/";
loadAllRent();
...
customerValidations.push({
    reg: regExName, field: $('#userName'), error: 'Customer  Name Pattern is Wrong'
});
customerValidations.push({
...
$("#userName,#customerContactNo,#customerAddress,#customerDriverEmail,#customerNic,#customerLicence,#customerUserName,#customerPassword").on('keydown', function (event) {
    if (event.key === "Tab") {
        event.preventDefault();
    }
});

$("#userName,#customerContactNo,#customerAddress,#customerDriverEmail,#customerNic,#customerLicence,#customerUserName,#customerPassword").on('keyup', function (event) {
    checkValidity(customerValidations);
});

$("#userName,#customerContactNo,#customerAddress,#customerDriverEmail,#customerNic,#customerLicence,#customerUserName,#customerPassword").on('blur', function (event) {
    checkValidity(customerValidations);
});


$("#userName").on('keydown', function (event) {
    if (event.key === "Enter" && check(regExName, $("#userName"))) {
