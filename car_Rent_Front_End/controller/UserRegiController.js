let userBaseUrl = "http://localhost:8080/car_Rent_Back_End_war/";
 ...
 * */
function setTextFieldValues(name, contact_No, address, email, nic, license_No, nic_Img, license_Img, user_Name, password) {
    $("#Name").val(name);
    ...
    if (event.which === 13) {
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                let row = "<tr><td>" + res.user_Id + "</td><td>" + res.name.name + "</td><td>" + res.contact_No + "</td><td>" + res.address + "</td><td>" + res.email + "</td><td>" + res.nic + "</td><td>" + res.license_No + "</td><td>" + res.user.role_Type + "</td><td>" + res.user.user_Name + "</td><td>" + res.user.password + "</td></tr>";
                $("#customerTable").append(row);
            },
            error: function (error) {
                loadAllRegUsers();
...
function blindClickEvents() {
    $("#customerTable>tr").on("click", function () {
        ...
        console.log(user_Id, name, address, contact_No, email, nic, license_No, role_Type, user_Name, password);
...
customerValidations.push({
    reg: regExName, field: $('#Name'), error: 'Customer  Name Pattern is Wrong'
});
customerValidations.push({
...
$("#Name,#contact_No,#address,#email,#nic,#license_No,#user_Name,#password").on('keydown', function (event) {
    if (event.key === "Tab") {
        event.preventDefault();
    }
});

$("#Name,#contact_No,#address,#email,#nic,#license_No,#user_Name,#password").on('keyup', function (event) {
    checkValidity(customerValidations);
});

$("#Name,#contact_No,#address,#email,#nic,#license_No,#user_Name,#password").on('blur', function (event) {
    checkValidity(customerValidations);
});
